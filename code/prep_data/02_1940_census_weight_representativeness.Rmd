...
Summary: Assess sample representativeness and construct raking weights. 
...
## read in 1940 census 
census_1940 <- fread("~/workspace/data/1940/P.tsv", select = c("HISTID", "SERIALP", "AGE", "INCWAGE", "SEX", "EDUC", "RACE", "RELATE", "MARST", "SEI")) %>% 
  janitor::clean_names()

## read in 1940 census household
census_1940_h <- fread("~/workspace/data/1940/H.tsv", select = c("SERIAL", "STATEFIP", "OWNERSHP", "URBAN", "REGION")) %>% 
  janitor::clean_names()
```
...
rep_plot <- combined_characteristics %>% 
  ...
  theme_bw() +
  theme(legend.position=c(.9223, .952),
...
ggsave(rep_plot, filename = here("figures/fig1_representativeness_plot.png"), height = 6, width = 9)
```
...
fwrite(dmf_clean_weights, here("data", "censoc_dmf_clean_weighted.csv"))
```
