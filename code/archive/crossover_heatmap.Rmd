...
dmf <- dmf %>% 
  ...
  left_join(dmf_new %>% select(histid = HISTID, weight_new = weight))

dmf <- dmf %>% filter(byear %in% 1890:1905)
```
...
calculate_ln_qx_weighted <- function(data) {
  data %>%
    ...
    summarize(deaths = sum(weight_new), .groups = "drop", ) %>%
...
dmf_filter <- dmf %>% 
  ...
  mutate(weight_new = weight_new / mean(weight_new, na.rm = T)) %>% 
  group_by(race) %>% 
  mutate(weight_new = case_when(is.na(weight_new) ~ mean(weight_new, na.rm = T), TRUE ~ weight_new))

ln_qx_point_estimate <- dmf %>% 
  calculate_ln_qx() %>% 
