...
## Read in dmf 
dmf <- fread(here("data", "censoc_dmf_clean_weighted.csv"))
...
# Save plots
ggsave(plot = pooled_bwcrossover, filename = here("figures", "fig2_bw_crossover_1890_1905.png"), width = 8.3, height = 4.5, bg = "white")
```
...
## BW-Crossover with NCHS weights 
...
## BW-Crossover with 1940 Census weights 
...
## BW Crossover 1940 Census weights X non-heaped

```{r}
## restrict to exact match 
qx_point_estimate_weighted <- dmf_filter %>% 
  filter(byear %in% c(1891:1894, 1896:1899, 1901:1904)) %>% 
  calculate_qx(weight_var = "weight_1940_census") %>% 
  group_by(death_age, race) %>%
...
# Plot
pooled_bwcrossover_census_weights_nonheaped <- qx_uncertainty_point_estimate %>%
  filter(death_age %in% 75:100, race %in% c("White Men", "Black Men")) %>%
  ggplot(aes(x = death_age, y = qx_point_estimate, color = race)) +
  geom_line() +
  geom_pointrange(aes(ymin = lower_ci, ymax = upper_ci, shape = race), size = 0.75, fill = "white") +
...
ratio_plot_census_weights_nonheaped <- ggplot(mort_ratio, aes(x = death_age, y = bw_ratio)) +
  geom_point(size = 3, alpha = 0.5) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  geom_smooth(method = "lm", se = F) + 
  labs(
...
## BW Crossover 1940 Census weights X exact match

```{r}
## 1940 Census weights X non-heaped
qx_point_estimate_weighted <- dmf_exact_match %>% 
  calculate_qx(weight_var = "weight_1940_census") %>% 
  group_by(death_age, race) %>%
  summarize(ln_qx_point_estimate = mean(ln_qx, na.rm = TRUE),
            qx_point_estimate = mean(qx, na.rm = TRUE))
...
# Plot
pooled_bwcrossover_census_weights_exactmatch <- qx_uncertainty_point_estimate %>%
  filter(death_age %in% 75:100, race %in% c("White Men", "Black Men")) %>%
  ggplot(aes(x = death_age, y = qx_point_estimate, color = race)) +
  geom_line() +
  geom_pointrange(aes(ymin = lower_ci, ymax = upper_ci, shape = race), size = 0.75, fill = "white") +
...
ratio_plot_census_weights_exactmatch <- ggplot(mort_ratio, aes(x = death_age, y = bw_ratio)) +
  geom_point(size = 3, alpha = 0.5) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  geom_smooth(method = "lm", se = F) + 
  labs(
...
```{r}
crossover_census_weights_checks <- cowplot::plot_grid(pooled_bwcrossover_census_weights_nonheaped, ratio_plot_census_weights_nonheaped, pooled_bwcrossover_census_weights_exactmatch, ratio_plot_census_weights_exactmatch,  labels = "AUTO")

ggsave(plot = crossover_census_weights_checks, filename = here("figures", "appendix_crossover_census_weights_1940census_weights.png"), width = 12, height = 8, bg = "white")
```


```{r}
fig3_robustness <- cowplot::plot_grid(pooled_bwcrossover_nonheaped, ratio_plot_nonheaped, pooled_bwcrossover_exact, ratio_plot_exact_match,pooled_bwcrossover_nchs_weights,  ratio_plot_nchs_weights, pooled_bwcrossover_census_weights, ratio_plot_census_weights, ncol = 2, labels = "AUTO")

ggsave(plot = fig3_robustness, filename = here("figures", "fig5_bw_crossover_1890_1905_robustness.png"), width = 12, height = 14, bg = "white")
```
...
df_wide <- df %>%
  ...
  mutate(diff_bw = `Black Men` / `White Men` )
...
crossover_by_cohort <- ggplot(df_wide, aes(x = death_age, y = diff_bw, group = byear)) +
  ...
  ggsave(plot = crossover_by_cohort, filename = here("figures", "fig3_bw_crossover_1890_1905_by_cohort.png"), width = 10, height = 10, bg = "white")
```
...
df_summary <- ln_qx_point_estimate %>%
  ...
  pivot_wider(names_from = race, values_from = qx) %>%
  mutate(
    diff_bw = `Black Men` - `White Men`,
    pct_diff = 100 * diff_bw / `White Men`
  )
